            ...
            try {
                ...
                for (final String str : header) {
                    if (nameToChrom.containsKey(str)) {
                        ...
                        Logger.getLogger(MaltcmsPeakFinderImporter.class.getName()).log(Level.FINE, "Found match for column {0} in chromatogram {1}", new Object[]{str, cd.getResourceLocation()});
                        indexToChrom.put(index, cd);
                        chromToPeaks.put(cd, project.getPeaks(cd));
                    } else {
                        Logger.getLogger(MaltcmsPeakFinderImporter.class.getName()).log(Level.WARNING, "Warning: could not match chromatogram in alignment file header {0}", str);
                    }
                    index++;
                }
                            ...
                            if (chrom != null) {
                                ...
                                if (sourceChrom != null) {
                                    ...
                                    if (ipad != null) {
                                        ...
                                        Logger.getLogger(MaltcmsPeakFinderImporter.class.getName()).log(Level.WARNING, "Warning: Peak at index {0} in file {1} could not be matched!", new Object[]{peakIndex, indexToChrom.get(colIdx).getDisplayName()});
                                    }
                                } else {
                                    Logger.getLogger(MaltcmsPeakFinderImporter.class.getName()).log(Level.WARNING, "Warning: Source Chromatogram at column index {0} was not found in project!", colIdx);
                                }
                            } else {
                                Logger.getLogger(MaltcmsPeakFinderImporter.class.getName()).log(Level.WARNING, "Warning: Chromatogram at column index {0} was not found in project!", colIdx);
                            }
                        }
                        colIdx++;
                    }
            ...
            for (IPeakAnnotationDescriptor ipad : p.getMembers()) {
                ...
                try {
                    ...
                    if (alignedPeakRt == queryRt) {
                        Logger.getLogger(MaltcmsPeakFinderImporter.class.getName()).log(Level.FINE, "matched peak to index {0}", index);
                        return ipad;
                    }
                } catch (ArrayIndexOutOfBoundsException aio) {
                    Logger.getLogger(MaltcmsPeakFinderImporter.class.getName()).log(Level.WARNING, "Match index for peak out of bounds!");
                }
            }
        }
        Logger.getLogger(MaltcmsPeakFinderImporter.class.getName()).log(Level.WARNING, "No match found!");
        return null;
    }
}
