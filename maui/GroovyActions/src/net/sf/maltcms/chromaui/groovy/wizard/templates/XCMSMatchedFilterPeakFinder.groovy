...
/**
 * This is a groovy based template for a peak-finder using XCMS's matched filter
 * method. It creates an R script file compatible with the Rscript program.
 * 
 * @author ${user}
 */

public class ${name} implements RawDataGroovyScript {
...
"""
    ...
    public void create(
            ...
            Collection<CDFDataObject> dobjects) {
    ...
    private int createCall(
            File outdir,
            File rscriptLocation,
        ...
        for(CDFDataObject it:dobjects) {
            ...
            File rscriptExeLocation = RserveConnectionFactory.getInstance().getRscriptLocation()
            if(rscriptExeLocation==null) {
                throw new NullPointerException("Please define Rscript location in Tools->Options->Miscellaneous->Rserve");
            }
    ...
    public void run() {
        ...
        try{
            ...
            //create map matching report names to chromatograms
            LinkedHashMap<String, IChromatogramDescriptor> chromatograms = 
                createChromatogramMap(project, writer);
            
            def jobLines = dataObjects.size()
            //create the r script within the file system
            ...
            //match chromatograms to reports, by name
            ...
            //create domain results for Maui
    ...
    private void importPeaks(ProgressHandle progressHandle,
            Map<String,File> reports,
            Map<String, IChromatogramDescriptor> chromatograms,
            IToolDescriptor trd,
            File importDir,
            writer) {
    ...
    private Map<String, File> mapReports(
            Map<String, IChromatogramDescriptor> chromatograms,
            File outputDir,
            writer) {
    ...
    private Map<String, IChromatogramDescriptor> createChromatogramMap(
            IChromAUIProject project,
            writer) {
        Map<String, IChromatogramDescriptor> chromatograms = new LinkedHashMap<String, IChromatogramDescriptor>();
