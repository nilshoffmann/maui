...
public class DatabaseDefragmentRunnable extends AProgressAwareRunnable {

    private final Db4oGeneralSettingsOptionsPanelController controller;

    public DatabaseDefragmentRunnable(Db4oGeneralSettingsOptionsPanelController controller) {
        this.controller = controller;
    }

    @Override
    public void run() {
        Logger.getLogger(getClass().getName()).info("Defragmenting databases!");
        ...
        final Collection<? extends Project> selectedProjects = 
            Projects.getSelectedOpenProjects(
                ...
                "Select Projects for Defragmentation"
            );
        //close all projects
        OpenProjects.getDefault().close(projects);
        RequestProcessor rp = new RequestProcessor("Database Defragmentation", 1);
