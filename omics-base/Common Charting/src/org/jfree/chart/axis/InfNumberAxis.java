 ...
 * 04-Oct-2002 : Moved standardTickUnits from InfNumberAxis --> ValueAxis (DG);
...
import static org.jfree.chart.axis.Axis.DEFAULT_AXIS_LINE_STROKE;
 ...
 * this) by setting the
 * <code>autoRangeIncludesZero</code> flag to
 * <code>true</code>.
 * <P>
 * The
 * <code>InfNumberAxis</code> class has a mechanism for automatically selecting
 ...
 */
public class InfNumberAxis extends ValueAxis implements Cloneable, Serializable {
	 ...
	 * The default value for the autoRangeIncludesZero flag.
	 */
	public static final boolean DEFAULT_AUTO_RANGE_INCLUDES_INF = true;
	/**
	...
	/**
	 * A flag that affects the axis range when the range is determined
	 * automatically. If the auto range does NOT include Inf and this flag is
	 * TRUE, then the range is changed to include Inf.
	 */
	private boolean autoRangeIncludesInf;
	/**
	 ...
	 */
	public InfNumberAxis(String label) {
		super(label, InfNumberAxis.createStandardTickUnits());
		this.rangeType = RangeType.FULL;
		this.autoRangeIncludesZero = DEFAULT_AUTO_RANGE_INCLUDES_ZERO;
		this.autoRangeIncludesInf = DEFAULT_AUTO_RANGE_INCLUDES_INF;
	 ...
	 * <code>true</code>, the axis range is recalculated.
	 * <p>
	...
	/**
	 * Returns the flag that indicates whether or not the automatic axis range
	 * (if indeed it is determined automatically) is forced to include Inf.
	 *
	 * @return The flag.
	 */
	public boolean getAutoRangeIncludesInf() {
		return this.autoRangeIncludesInf;
	}

	/**
	 * Sets the flag that indicates whether or not the axis range, if
	 * automatically calculated, is forced to include Inf.
	 * <p>
	 * If the flag is changed to
	 * <code>true</code>, the axis range is recalculated.
	 * <p>
	 ...
	 * @param flag the new value of the flag.
	 *
	 * @see #getAutoRangeIncludesInf()
	 */
	public void setAutoRangeIncludesInf(boolean flag) {
		if (this.autoRangeIncludesInf != flag) {
			this.autoRangeIncludesInf = flag;
			if (isAutoRange()) {
				autoAdjustRange();
			}
			notifyListeners(new AxisChangeEvent(this));
	 ...
	 * <p>
	 * Note: if the
	 * <code>autoTickUnitSelection</code> flag is
	 ...
	 * <P>
	 * The marker band is optional, leave it set to
	 * <code>null</code> if you don't require it.
	 *
...
//        }
	...
	public Object clone() throws CloneNotSupportedException {
		InfNumberAxis clone = (InfNumberAxis) super.clone();
	...
	public boolean equals(Object obj) {
		...
		if (!(obj instanceof InfNumberAxis)) {
			return false;
		}
		InfNumberAxis that = (InfNumberAxis) obj;
	...
	protected void drawAxisLine(Graphics2D g2, double cursor, Rectangle2D dataArea, RectangleEdge edge, Stroke stroke) {
    ...
    protected void drawAxisLine(Graphics2D g2, double cursor,
		...
		double xdomainMinValue = java2DToValue(dataArea.getMinX(), dataArea, edge);
		double xdomainMaxValue = java2DToValue(dataArea.getMaxX(), dataArea, edge);
		Range r = getRange();
		double rangeLower = getLowerBound();
		double rangeLowerInView = valueToJava2D(rangeLower, dataArea, edge);
		double rangeUpper = getUpperBound();
		double rangeUpperInView = valueToJava2D(rangeUpper, dataArea, edge);
		
		double insetLeft = 0;
		double insetRight = 0;
		double insets = 0.1*dataArea.getWidth();
		Rectangle2D left = null, middle, right = null;
		if(xdomainMinValue>rangeLower) {
			insetLeft = 0.05*dataArea.getWidth();
			left = new Rectangle2D.Double(dataArea.getMinX(),dataArea.getMinY(),insetLeft,dataArea.getHeight());
		}
		if(xdomainMaxValue>rangeUpper) {
			insetRight = 0.05*dataArea.getWidth();
			right = new Rectangle2D.Double(dataArea.getMaxX()-insetRight,dataArea.getMinY(),insetRight,dataArea.getHeight());	
		}
		middle = new Rectangle2D.Double(dataArea.getMinX()+insets,dataArea.getMinY(),dataArea.getWidth()-(insetLeft+insetRight),dataArea.getHeight());
		
		if(left!=null)drawAxisLine(g2, cursor, left, edge, stroke);
		drawAxisLine(g2, cursor, middle, edge, DEFAULT_AXIS_LINE_STROKE);
		if(right!=null)drawAxisLine(g2, cursor, right, edge, stroke);
//		dataArea = new Rectangle2D.Double(dataArea.getMinX()+10, dataArea.getMinY(), dataArea.getWidth()-20, dataArea.getHeight());
//        Line2D axisLine = null;
//        if (edge == RectangleEdge.TOP) {
//            axisLine = new Line2D.Double(dataArea.getX(), cursor,
//                    dataArea.getMaxX(), cursor);
...
//		AxisSpace axisSpace = super.reserveSpace(g2, plot, plotArea, edge, space); 
//		if(edge==RectangleEdge.LEFT || edge==RectangleEdge.RIGHT) {
//			axisSpace.add(plotArea.getHeight()*0.5, edge);
//		}else{
//			axisSpace.add(plotArea.getWidth()*0.5, edge);
//		}
//		return axisSpace;
//		
//	}
